...
import stokes_tester
import os
import re

def run_data(filename_Q, filename_U, filename_I, plot_variable=None,
             save_show='show', graph_selection='all', file_extension='.png', projection='ortho',
    ...
    map_filename = path_map_filename + file_extension
    path_new_histogram_filename = os.path.join(directory, histogram_filename)
    histogram_filename = path_new_histogram_filename + file_extension
    path_new_drapery_filename = os.path.join(directory, drapery_filename)
    drapery_filename = path_new_drapery_filename + file_extension

    #print histogram_filename, map_filename, drapery_filename
    ...
    if graph_selection is 'all':
        if plot_variable is not None:
            plothealpix_map.mapping(ra, dec, map_data_var, plot_variable, obsID=obsID,
                                    map_file_name=map_filename, projection=projection, save_show=save_show)

        else:
            raise ValueError('Must have mapping variable and variable name inputs to plot map.')

        stokes_tester.plotstokesQU(obsID, x_stokes, y_stokes, ra, dec, map_var='theta',
                          new_histogram_filename=histogram_filename, directory=None,
                          save_show=save_show, projection=projection)

        lic_maps.LIC(obsID, x_stokes, y_stokes, ra, dec, 100, 500, length=1000, full_image=False,
                     disp_drapery=save_show, name_of_plot=drapery_filename)
    ...
    # Plot of the direction of polarization created with the line integral convolution method
        ...
        lic_maps.LIC(x_stokes, y_stokes, ra, dec, 100, 500, length=1000, full_image=False, disp_drapery=save_show,
                     name_of_plot=drapery_filename)
